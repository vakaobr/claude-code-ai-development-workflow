...
Test authenticated endpoints for Insecure Direct Object Reference flaws - cases where the application grants access to a resource based on a
...
3. IDOR testing requires two authenticated sessions for different users.
   Confirm both sets of test credentials are approved for this asset -    check `requires_auth_testing_credentials` and
...
- **Public resources**: The ID refers to a resource intended to be public
  (public user profile, marketing document). Filing these creates noise -   confirm sensitivity with the product team first.
